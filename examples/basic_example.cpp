...
int main() {
    ...
    msgbus::MessageBus bus(msgbus::kDefaultQueueCapacity, 4, msgbus::FullPolicy::DropNewest);
    bus.start();

    std::cout << "Policy: "
              << (bus.policy() == msgbus::FullPolicy::DropNewest ? "DropNewest" : "Other")
              << "\n\n";

    // --- Exact topic subscriptions ---
    auto sub1 = bus.subscribe<SensorData>("sensor/temperature", [](const SensorData& data) {
        std::cout << "[Temp] Sensor " << data.sensor_id << " = " << data.value << "°C\n";
    });

    auto sub2 = bus.subscribe<SensorData>("sensor/humidity", [](const SensorData& data) {
        std::cout << "[Humidity] Sensor " << data.sensor_id << " = " << data.value << "%\n";
    });

    // --- Wildcard subscription: '*' matches one level ---
    // Receives ALL sensor types (temperature, humidity, pressure, ...)
    auto sub_all_sensors = bus.subscribe<SensorData>("sensor/*", [](const SensorData& data) {
        std::cout << "[AllSensors] Sensor " << data.sensor_id << " value: " << data.value << "\n";
    });
    ...
    auto sub_system = bus.subscribe<LogEvent>("system/#", [](const LogEvent& event) {
        std::cout << "[System] [" << event.level << "] " << event.message << "\n";
    });
